...
/**
 * MCP stdio smoke test against the real bin entry, with mock Docker transport.
 *
...
async function packForNpx() {
  const dir = mkdtempSync(join(tmpdir(), "mcp-docker-pack-"));
...
async function main() {
  ...
  if (mode === "npx") {
    ...
    server = {
      command: "npx",
      args: ["-y", `--package=${packed.tarball}`, "mcp-docker-agentic"],
      env: {
        ...process.env,
        MCP_DOCKER_AGENTIC_MOCK: "1",
        npm_config_yes: "true",
      },
    };
  } else if (mode === "node") {
